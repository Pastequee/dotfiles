...
2. Installs: `[tools]` for versioned CLIs, `[bootstrap.packages]` for `brew-cask:` / `mas:` / `brew:`. Anything that needs a vendor script or a Ruby-only tap goes in `scripts/install-extras.sh` and must skip if already installed.
